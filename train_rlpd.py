...
parser.add_argument("--ensemble_size", type=int, default=2,
                    help="2 = standard twin-Q (default); 10 = REDQ-style ensemble for architecture-matched comparison with RE-SAC")
...
out_dir = os.path.join(_HERE, "experiment_output", f"rlpd{_ratio_tag}{_arch_tag}{_user_tag}_seed{args.seed}_{_ts}")
...
if USE_ENSEMBLE:
    ...
    print(f"RLPD with REDQ-style ensemble: E={args.ensemble_size}, M={args.redq_M}")
else:
    ...
    print("RLPD with twin-Q (E=2)")
vf         = BusEmbeddingVFunction(state_dim, hidden_sz, emb_tmpl.clone())
# RLPD: random init (no pretrained weights loaded)

dev = torch.device(args.device)
if USE_ENSEMBLE:
    nets_to_dev = [policy, qfs, target_qfs, vf]
else:
    nets_to_dev = [policy, qf1, qf2, target_qf1, target_qf2, vf]
...
def mixed_sac_step(batch):
    ...
    with torch.no_grad():
        ...
        if USE_ENSEMBLE:
            q_all = qfs(obs, pi_a)  # [E, B]
            q_pi = q_all.min(dim=0).values
        else:
    ...
    if USE_ENSEMBLE:
        # REDQ-style: min over M randomly sampled heads
        q_all2 = qfs(obs, pi_a2)  # [E, B]
